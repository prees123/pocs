...
export const CategoryTile = (props) => {
  function categoryButtons () {
    ...
    if (_.isEmpty(props.categories)) {
      return (<div>
        <Grid>
          <Button variant="outlined">Categories Not Found</Button>
        </Grid>
      </div>)
    } else {
      return (<div>
        <Grid className={classes.grid} container justify="center" alignItems="center" spacing={16}>
          {
            Object.keys(props.categories).map(function (id, name) {
              ...
              let parentCategoryId = props.categories[id].parentCategoryId
              let topLevelCategoryName = props.topLevelCategoryName
              if (categoryName.includes('&')) {
                categoryName = categoryName.replace('&', 'and')
              }
              const linkUrl = `/category/${topLevelCategoryName}/${categoryId}/?id=${categoryId}&name=${categoryName}&parentCategoryId=${parentCategoryId}`
...
export default withStyles(styles)(withRouter(CategoryTile))
